    ...
    constructor(props) {
      ...
      this.clickEdit = () => {
        if (this.selfState.editing) {
          ...
          this.selfState.editableColumns = observable(
            JSON.parse(JSON.stringify(this.selfState.columns))
          ).map(column => ({
            key: column,
            name: column,
            editable: true,
      ...
      this.saveTableChange = () => {
        if (window.confirm("保存更改？")) {
          this.selfState.handling = true;
          this.lastSaveState.data = JSON.parse(
            JSON.stringify(this.selfState.editableRows)
          );
          this.lastSaveState.columns = JSON.parse(
            JSON.stringify(this.selfState.columns)
          );
      ...
      ipcRenderer.on(SAVE_TABLE_DATA_CHANGE_RETURN, (_, { ok, err }) => {
        this.selfState.handling = false;
        if (ok) {
      ...
      ipcRenderer.on(DELETE_TABLE_RETURN, () => {
        ...
        this.selfState.handling = false;
        this.selfState.table = "";
        this.selfState.originalRows = this.selfState.editableRows = this.selfState.data = this.selfState.editableColumns = this.selfState.columns = observable(
          []
        );
        this.selfState.newTableName = "";
        this.selfState.editing = false;
        ...
        for (let j = 0; j < store.classes.length; j++) {
          if (
            (i = store.classes[j].tables.findIndex(t => t === table)) !== -1
          ) {
        ...
        >
          ...
          >
            <Input
              addonBefore={"表名"}
              addonAfter={
                <a style={{ width: "100%" }} onClick={this.changeTableName}>
                  修改表名
                </a>
              }
              onChange={e => {
            ...
            <Button onClick={this.clickEdit}>
              {this.selfState.editing ? "放弃更改" : "编辑表格"}
            </Button>
            {this.selfState.editing ? (
              <Button onClick={this.saveTableChange}>保存更改</Button>
            ) : null}
          ...
          {!this.selfState.editing ? (
            <ReactTable
              ...
              columns={this.selfState.columns.slice().map(col => ({
                Header: col,
                accessor: col,
