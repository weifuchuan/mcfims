            ...
            searchResult.length > 0 ?
              (
                <span style={{ fontSize: "large" }}>
                  搜索结果：<span>总共<span style={{color:"#3d27ff"}}>{searchResult.reduce((sum, item) => sum + item.rows.length, 0)}</span>条；</span>{
                  searchResult.map((item,i) => {
                    return (
                      <span key={i.toString()}>{item.name}：<span style={{color:"#3d27ff"}}>{item.rows.length}</span>条；</span>
                    );
                  })
                }</span>
              ) : null
